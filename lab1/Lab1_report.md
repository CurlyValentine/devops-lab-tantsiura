...
- Установлен и проверен Docker через запуск тестового контейнера `hello-world`.  
- Освоены базовые команды (`docker ps`, `docker images`, `docker stop`, `docker rm`, `docker rmi`).  
- Выполнен запуск контейнера с образом Ubuntu и установка пакета `curl`.  
- Настроен и протестирован контейнер с веб-сервером `nginx`.  
- Освоено управление контейнерами (просмотр, остановка, удаление, повторный запуск).  
- Изучена работа с томами (`volumes`) и подтверждено сохранение данных после удаления контейнера.  
- Создан собственный образ на основе `python:3.9-slim`, содержащий Flask-приложение.  
- Образ успешно собран и запущен; приложение доступно по адресу [http://localhost:5000](http://localhost:5000), что подтверждается ответом:  
  **Hello from Docker!**
